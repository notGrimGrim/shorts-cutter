...
def _run(engine, track, language):
    ...
    options = dict(
        ...
        # звука 1700–1740 с:
        #
        #   с подсказкой из названия  →  6 слов: «ПОБЕДИ НА ТУРНИРЕ.
        #                                 ПОБЕДА НА ТУРНИРЕ.»
        #   без подсказки             → 44 слова: «они контролили там
        #                                 просто, это я свой, уходи на б…»
        #
        # То есть подсказка не дополняла расшифровку, а подменяла её: модель
        # залипала на тексте промпта и теряла до 80% речи, а название ролика
        # («ПОБЕДА НА ТУРНИРЕ ПО CS2») размазывалось по всему ролику как
        # будто это и было сказано. Нейтральная подсказка вредит так же
