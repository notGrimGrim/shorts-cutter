...
def _form(path, language, hint):
    ...
    field("temperature", "0")
    # prompt НЕ отправляем, хотя API его принимает. За ним стоит тот же
    # механизм initial_prompt, что и у местной модели, и та же беда: на
    # стриме CS2 подсказка из названия ролика («ПОБЕДА НА ТУРНИРЕ ПО CS2»)
