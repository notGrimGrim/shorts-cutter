...
# Плашка сверху висит весь шортс, а не первые секунды. Зритель приходит на
# середине пролистывания, и вопрос ему нужен именно в этот момент, а не тем,
# кто смотрел с нулевой секунды. Месту она не мешает: надпись сверху,
# субтитры снизу.
HOOK_SECONDS = 0.0
...
def write_ass(path, phrases, start, end, hook=None, keys=(), hook_margin=0,
    ...
    if hook:
        # HOOK_SECONDS = 0 значит «весь кусок»; иначе столько секунд с начала.
        held = min(HOOK_SECONDS, end - start) if HOOK_SECONDS else end - start
        size = hook_size(hook)
