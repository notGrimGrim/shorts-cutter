...
# Но не бесконечно: даём договорить, а не рассказать ещё историю. Из двух
# потолков — «вдвое от заказа» и «плюс столько секунд» — берётся МЕНЬШИЙ.
#
# 20 → 35 → 75: и 20, и 35 упирались раньше, чем ответ договаривался.
#
# Здесь был спор с самим собой. Комментарий раньше обещал запас в секундах,
# а код брал min(), то есть на коротком заказе (--max 30) запас выходил не
# 75 с, а 30. Замер показал, что прав код, а не комментарий: с max() заказ
# «25–30 секунд» давал куски по 80 с — человек просил полминуты и получал
# полторы. Раскрытая тема важнее длины, но не настолько, чтобы втрое
# нарушать прямую просьбу. На обычном заказе (90 с) обе формулы совпадают.
ANSWER_ROOM = 75.0
